...
def maskOutGreens( answer, lockedGreens ):
    maskedAnswer = list(answer.lower())
    if( len(answer) != 5 ):
        print(f"[maskOutGreens] answer should be 5 letters long")
        exit(1)
    if( len(lockedGreens) != 5 ):
        print(f"[maskOutGreens] locked greens should be 5 chars long")
        exit(1)
    # Mask out greens.
    for greenIdx in range(5):
        if lockedGreens[greenIdx] != ' ':
            maskedAnswer[greenIdx] = '-'
    return ''.join(maskedAnswer)
...
if answer not in validWords:
    print(f"{answer} is not a valid word.")
    exit(1)
...
for turn in range( len(guesses) ):
    ...
    outcome = ""
    maskedAnswer = maskOutGreens( answer, lockedGreens )
    for idx in range(5):
        # current letter's the same between guess and answer = UPPER-CASE
        if guess[idx] == answer[idx].lower():
            outcome += answer[idx].upper()
        # current letter's in answer but not found yet
        elif guess[idx] in maskedAnswer:
            outcome += guess[idx]
        # current letter's not in answer = ' '
        else:
            outcome += ' '

    if outcome == answer.upper():
